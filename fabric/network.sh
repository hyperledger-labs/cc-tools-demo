...
function copyRestCerts() {
  REST_CERTS_FOLDER=./organizations/rest-certs
  ORGNAME=$1
  PRIVATE_KEY_PATH=$(ls $PWD/organizations/peerOrganizations/${ORGNAME}/users/Admin@${ORGNAME}/msp/keystore/*_sk)
  ...
  ADMINCERT_PATH=$PWD/organizations/peerOrganizations/${ORGNAME}/users/Admin@${ORGNAME}/msp/signcerts/Admin@${ORGNAME}-cert.pem
  cp $ADMINCERT_PATH $REST_CERTS_FOLDER/${ORGNAME}/admin.cert
  # Copy cacert to rest-certs
  CACERT_PATH=$PWD/organizations/peerOrganizations/${ORGNAME}/users/Admin@${ORGNAME}/tls/ca.crt
